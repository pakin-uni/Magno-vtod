...
class MagnoFusionBlock(nn.Module):
    def __init__(self, in_channels, out_channels=32, stride=2, tau=2.0):
        super().__init__()

        # RGB and magnocellular conv paths
        self.rgb_conv = nn.Sequential(
            nn.Conv2d(3, out_channels, kernel_size=3, stride=stride, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )

        self.magno_conv = nn.Sequential(
            nn.Conv2d(1, out_channels, kernel_size=3, stride=stride, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )
        ...
        # Gaussian blur layers (wrapped torchvision blur)
        self.blur_compress = GaussianBlurLayer(kernel_size=9, sigma=2.0)
        self.blur_on = GaussianBlurLayer(kernel_size=5, sigma=1.0)
        self.blur_off = GaussianBlurLayer(kernel_size=5, sigma=7.0)
        self.blur_ganglion = GaussianBlurLayer(kernel_size=5, sigma=1.5)

        # Temporal buffers (for amacrine cell dynamics)
        self.Xon_prev = None
        self.Xoff_prev = None
        self.Ypos_on_prev = None
        self.Ypos_off_prev = None

        self.lam = np.exp(-1.0 / tau)

    def photoreceptor_compression(self, gray, V0=0.9, Vmax=1.0):
        blur = self.blur_compress(gray)
        R0 = V0 * blur + Vmax * (1 - V0)
        return (gray / (gray + R0)) * (Vmax + R0)

    def opl_filter(self, gray):
        Fph = self.blur_on(gray)
        Fh = self.blur_off(gray)
        return torch.clamp(Fph - Fh, min=0), torch.clamp(Fh - Fph, min=0)

    def amacrine_filter(self, on, off):
        if self.Xon_prev is not None:
            if self.Xon_prev.shape != on.shape:
                self.reset_states()  # auto-reset if shape changed
        if self.Xon_prev is None:
            self.Xon_prev = on.clone()
            self.Xoff_prev = off.clone()
            self.Ypos_on_prev = torch.zeros_like(on)
            self.Ypos_off_prev = torch.zeros_like(off)

        Yon = self.lam * (self.Ypos_on_prev + on - self.Xon_prev)
        Ypos_on = torch.where(Yon > 0, Yon, self.Ypos_on_prev)

        Yoff = self.lam * (self.Ypos_off_prev + off - self.Xoff_prev)
        Ypos_off = torch.where(Yoff > 0, Yoff, self.Ypos_off_prev)

        self.Xon_prev = on.clone()
        self.Xoff_prev = off.clone()
        self.Ypos_on_prev = Ypos_on.clone()
        self.Ypos_off_prev = Ypos_off.clone()

        return torch.max(Yon, Yoff)

    def ganglion_filter(self, amacrine_out, gain=2.0):
        blur = self.blur_ganglion(amacrine_out)
        contrast = amacrine_out - blur
        return torch.sigmoid(gain * contrast)  # Smooth saturation

    def forward(self, rgb):  # Input: [B, 3, H, W]
        #print(f"[Magno] RGB input shape: {rgb.shape}")
        gray = 0.2989 * rgb[:, 0:1] + 0.587 * rgb[:, 1:2] + 0.114 * rgb[:, 2:3]

        compressed = self.photoreceptor_compression(gray) #Cph
        on, off = self.opl_filter(compressed) 
        motion = self.amacrine_filter(on, off) #A
        magno = self.ganglion_filter(motion)  # [B, 1, H, W]
        #print(f"[Magno] Motion map shape: {magno.shape}")
        rgb_feat = self.rgb_conv(rgb)
        mag_feat = self.magno_conv(magno)
        #print(f"[Magno] RGB conv out: {rgb_feat.shape}, Mag conv out: {mag_feat.shape}")

        fused = rgb_feat + mag_feat
        fused_out = self.fusion(fused)
        #print(f"[Magno] Output shape: {fused_out.shape}")
        return fused_out  # Output: [B, out_channels, H/stride, W/stride]
